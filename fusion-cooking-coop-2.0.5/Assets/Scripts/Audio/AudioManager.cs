...
public class AudioManager : MonoBehaviour
{
	...
	public static void Snapshot(string name, float duration = 0)
	{
		if (string.IsNullOrWhiteSpace(name))
		{
			Instance.defaultSnapshot.TransitionTo(duration);
			return;
		}

		try
		{
			Instance.masterMixer.FindSnapshot(name).TransitionTo(duration);
		}
		catch
		{
    ...
    public static void Play(string clip, AudioMixerGroup mixerTarget, Vector3? position = null, Vector2? disparity = null)
	{
		if (Instance.soundBank.TryGetAudio(clip, out AudioClip audioClip))
		{
			...
			if (disparity.HasValue)
			{
				src.pitch = Random.Range(disparity.Value.x, disparity.Value.y);
			}
			src.clip = audioClip;
	...
	public static void Play(string clip, MixerTarget mixerTarget, Vector3? position = null, Vector2? disparity = null)
	{
		Play(clip, Instance.GetMixerGroup(mixerTarget), position, disparity);
	}

	public static void Play(string clip, string mixerTarget, Vector3? position = null, Vector2? disparity = null)
	{
		Play(clip, Instance.GetMixerGroup(mixerTarget), position, disparity);
	}

	public static void Play(string clip, Vector3? position = null, Vector2? disparity = null)
	{
		Play(clip, MixerTarget.Default, position, disparity);
	}
	...
	{
		PlayMusic(musicOptions[Random.Range(0, musicOptions.Length)]);
	}
	...
	{
		float v = PlayerPrefs.GetFloat(pref, 0.5f);
		return v;
	}
	...
	public class BankKVPDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			EditorGUI.PropertyField(rect1, property.FindPropertyRelative(nameof(BankKVP.Key)), GUIContent.none);
			EditorGUI.PropertyField(rect2, property.FindPropertyRelative(nameof(BankKVP.Value)), GUIContent.none);
